...
import { Plus, Zap, MessageCircle, Trash2, Power, X, Loader2, AlertCircle, Send, Edit2, Check } from "lucide-react";
...
export function AdsAutomationsClient({
  ...
  const startEdit = (auto: AdsAutomation) => {
    setForm({
  ...
  const renderForm = (isEdit: boolean, editId?: string) => (
    <div className="bg-white rounded-[2rem] border border-[#1877F2]/20 shadow-lg shadow-[#1877F2]/5 p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-black text-slate-900">{isEdit ? "Edit Automation" : "New Ad Automation"}</h2>
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            onClick={isEdit && editId ? () => handleEdit(editId) : handleCreate}
            disabled={saving}
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* How it works */}
      {automations.length === 0 && !showForm && !editingId && (
        ...
        {automations.length > 0 ? (
          <div className="divide-y divide-slate-50">
            {automations.map((auto) =>
              editingId === auto.id ? (
                <div key={auto.id} className="p-5 md:p-6">
                  {renderForm(true, auto.id)}
                </div>
              ) : (
